...
const DoctorCardRow = ({ category, onSelect }) => {
  ...
  if (loading) return <div className="text-slate-500 text-[10px] animate-pulse uppercase tracking-[0.2em] mt-4 ml-4">Seeking Specialists...</div>;
  
  if (doctors.length === 0) return (
    <div className="p-4 glass-card rounded-2xl border border-rose-200 text-rose-500 text-xs italic bg-white">
      No specialists currently on duty. Please contact support.
    </div>
  );

  return (
    <div className="flex overflow-x-auto gap-4 pb-4 mt-4 no-scrollbar px-2 -mx-2">
      {doctors.map((doc, index) => (
        <motion.div
          ...
          className="flex-shrink-0 w-[230px] glass-card p-5 rounded-[24px] border border-slate-200 hover:border-blue-300 transition-all cursor-pointer group bg-white shadow-xl"
        ...
        >
          ...
          <div className="space-y-1 mb-4">
              <h4 className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{doc.name}</h4>
              <p className="text-[10px] text-slate-500 font-medium leading-tight">{doc.specialization}</p>
          </div>
          ...
          <button className="w-full mt-5 py-2.5 rounded-xl bg-blue-50 text-blue-600 text-[10px] font-bold uppercase tracking-widest border border-blue-200 group-hover:bg-blue-600 group-hover:text-white transition-all flex items-center justify-center">
