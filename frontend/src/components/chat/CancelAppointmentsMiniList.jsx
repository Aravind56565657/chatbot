...
const CancelAppointmentsMiniList = ({ appointments, onSelectCancel }) => {
  ...
  return (
    <div className="space-y-3 w-full max-w-[360px] mt-4">
      <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] px-2 flex items-center justify-between">
        <span>Your Active Appointments</span>
      ...
      {appointments.map((apt, idx) => {
        ...
        return (
          ...
          >
            ...
            <div className="mt-3">
              <button
                onClick={() => onSelectCancel && onSelectCancel(apt)}
                className="w-full py-2.5 bg-red-50 hover:bg-red-100 text-[10px] font-bold text-red-500 rounded-lg transition-colors border border-red-200"
              >
                Select to Cancel
              </button>
            </div>
...
export default CancelAppointmentsMiniList;
